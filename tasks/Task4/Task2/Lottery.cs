    ...
    class Scratch_card : IGambling
    {
        ...
        public void get_instructions()
        {
            string output = "In the lottery ticket " + Name + " you will scratch " + drawing +
                " sums. Your starting balance is " +balance+ "$. If a specific sum appears 3 time, you have won this sum once.\n";
        ...
        public List<int> generate_rand()
        {
            ...
            for (int i = 0; i < drawing; i++)
            {
                ...
                var count_occ = rand_list.FindAll(x => x == sums[index]).Count();
                if (count_occ < 3) { rand_list.Add(sums[index]); } else i--;
            }
            return rand_list;
        }
        ...
        public void play()
        {
            List<int> list = generate_rand();
            print_list(list, Name);
            check_winnings(list);
            print_new_balance();
        }

        public void print_new_balance()
        {
            Console.WriteLine($"Your new balance is: {balance}$\n");
        }

        private void check_winnings(List<int> sum_list)
        {
            var cnt = sum_list.GroupBy(x => x).Where(x => x.Count() == 3).Select(x => x.Key);
            if (cnt.Any())
            {
            ...
            {
                Console.WriteLine("Leider nichts gewonnen");
                balance -= bet;
            }
